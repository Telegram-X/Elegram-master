...
import sadegh.shamsi.ShamsiCalendar;
import sadegh.shamsi.ShamsiDate;
...
public class LocaleController {
    ...
    public static boolean onlyShamsi = false;
    ...
    public static LocaleController getInstance() {
        ...
        if (localInstance == null) {
            synchronized (LocaleController.class) {
                ...
                onlyShamsi = preferences.getBoolean("onlyShamsiDate", false);
            }
        }
...
//    }
    ...
    public static String formatDateChat(long j) {
        try {
            ...
            if (!language.toLowerCase().equals("fa") && !language.toLowerCase().equals("ku") && !onlyShamsi) {
    ...
    public static String formatAdsTime(int time) {
        String text ;
        int hours = time / 60 / 60;
        time -= hours * 60 * 60;
        int minutes = time / 60;
        ...
        String hour = f.format(hours);

        String minuts = f.format(minutes);

        String second = f.format(time);

        if (hours > 0){
            text = String.format("%s:%s:%s" , hour , minuts , second);
        }else if (minutes > 0){
            text = String.format("%s:%s" ,  minuts , second);
        }else {
            text = String.format("%s" , second);
        }

        return String.format(ApplicationLoader.applicationContext.getResources().getString(R.string.timeForAds) , text);
    }
...
//    }
    public static String stringForMessageListDate(long date) {
        try {
            ...
            String language = locale.getLanguage();
            if ( onlyShamsi || language.toLowerCase().equals("fa") || language.toLowerCase().equals("ku")) {
    ...
    public static String formatShortNumber(int number, int[] rounded) {
        ...
        if (true) {
            ...
            return formatter.getNumberInstance(Locale.ENGLISH).format(number);
        }
    ...
    public static boolean fromIran() {

        if (LocaleController.getInstance().getCurrentLocaleInfo().shortName.equalsIgnoreCase("fa") || (UserConfig.getInstance(UserConfig.selectedAccount).getClientPhone() != null && UserConfig.getInstance(UserConfig.selectedAccount).getClientPhone().startsWith("98"))) {
            return true;
        } else
            return false;
    }

    public static String formatVipTime(long time) {
        String text ;
        ...
        String month = f.format(months);

        String day = f.format(days);
        ...
        if (months > 100){
            text = getString("Ever", R.string.Ever);
        }else if (months > 0){
            text = String.format("%s %s , %s %s , %s %s" , month , getString("Months", R.string.Months) , day , getString("Days", R.string.Days) , hour ,getString("Hours", R.string.Hours));
        }else  if (days > 0){
            text = String.format("%s %s , %s:%s:%s" , day , getString("Days", R.string.Days), hour , minuts, second);
        }else if (hours > 0){
            text = String.format("%s:%s:%s" , hour , minuts , second);
        }else if (minutes > 0){
            text = String.format("%s:%s" ,  minuts , second);
        ...
        return String.format(ApplicationLoader.applicationContext.getResources().getString(R.string.timeForVIP) , text);
    }
