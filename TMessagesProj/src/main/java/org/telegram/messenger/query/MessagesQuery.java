...
public class MessagesQuery {
    ...
    private static MessageObject loadPinnedMessageInternal(final int channelId, final int mid, boolean returnValue) {
        try {
            ...
            SQLiteCursor cursor = MessagesStorage.getInstance(UserConfig.selectedAccount).getDatabase().queryFinalized(String.format(Locale.US, "SELECT data, mid, date FROM messages WHERE mid = %d", messageId));
            if (cursor.next()) {
            ...
            if (result == null) {
                cursor = MessagesStorage.getInstance(UserConfig.selectedAccount).getDatabase().queryFinalized(String.format(Locale.US, "SELECT data FROM chat_pinned WHERE uid = %d", channelId));
                if (cursor.next()) {
                    NativeByteBuffer data = cursor.byteBufferValue(0);
                    if (data != null) {
                        result = TLRPC.Message.TLdeserialize(data, data.readInt32(false), false);
            ...
            if (result == null) {
                ...
                req.channel = MessagesController.getInstance(UserConfig.selectedAccount).getInputChannel(channelId);
                req.id.add(mid);
                    ...
                    public void run(TLObject response, TLRPC.TL_error error) {
                        ...
                        if (error == null) {
                            ...
                            if (!messagesRes.messages.isEmpty()) {
                                ...
                                MessagesStorage.getInstance(UserConfig.selectedAccount).putUsersAndChats(messagesRes.users, messagesRes.chats, true, true);
                                savePinnedMessage(messagesRes.messages.get(0));
                                ok = true;
                            }
                        }
                        if (!ok) {
                            MessagesStorage.getInstance(UserConfig.selectedAccount).updateChannelPinnedMessage(channelId, 0);
                        }
                    }
                });
            } else {
        ...
        MessagesStorage.getInstance(UserConfig.selectedAccount).getStorageQueue().postRunnable(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    SQLitePreparedStatement state = MessagesStorage.getInstance(UserConfig.selectedAccount).getDatabase().executeFast("REPLACE INTO chat_pinned VALUES(?, ?, ?)");
    ...
    private static MessageObject broadcastPinnedMessage(final TLRPC.Message result, final ArrayList<TLRPC.User> users, final ArrayList<TLRPC.Chat> chats, final boolean isCache, boolean returnValue) {
        ...
        if (returnValue) {
            return new MessageObject(UserConfig.selectedAccount ,result, usersDict, chatsDict, false);
        } else {
            AndroidUtilities.runOnUIThread(new Runnable() {
                @Override
                public void run() {
                    MessagesController.getInstance(UserConfig.selectedAccount).putUsers(users, isCache);
                    MessagesController.getInstance(UserConfig.selectedAccount).putChats(chats, isCache);
                    NotificationCenter.getInstance(UserConfig.selectedAccount).postNotificationName(NotificationCenter.didLoadedPinnedMessage, new MessageObject(UserConfig.selectedAccount , result, usersDict, chatsDict, false));
                }
            });
        }
        return null;
    ...
    public static void loadReplyMessagesForMessages(final ArrayList<MessageObject> messages, final long dialogId) {
        if ((int) dialogId == 0) {
            ...
            MessagesStorage.getInstance(UserConfig.selectedAccount).getStorageQueue().postRunnable(new Runnable() {
                ...
                public void run() {
                    try {
                        SQLiteCursor cursor = MessagesStorage.getInstance(UserConfig.selectedAccount).getDatabase().queryFinalized(String.format(Locale.US, "SELECT m.data, m.mid, m.date, r.random_id FROM randoms as r INNER JOIN messages as m ON r.mid = m.mid WHERE r.random_id IN(%s)", TextUtils.join(",", replyMessages)));
                        while (cursor.next()) {
                            NativeByteBuffer data = cursor.byteBufferValue(0);
                            if (data != null) {
                        ...
                        AndroidUtilities.runOnUIThread(new Runnable() {
                            @Override
                            public void run() {
                                NotificationCenter.getInstance(UserConfig.selectedAccount).postNotificationName(NotificationCenter.didLoadedReplyMessages, dialogId);
                            }
                        });
                    } catch (Exception e) {
                        FileLog.e(e);
            ...
            MessagesStorage.getInstance(UserConfig.selectedAccount).getStorageQueue().postRunnable(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        SQLiteCursor cursor = MessagesStorage.getInstance(UserConfig.selectedAccount).getDatabase().queryFinalized(String.format(Locale.US, "SELECT data, mid, date FROM messages WHERE mid IN(%s)", stringBuilder.toString()));
                        while (cursor.next()) {
                            NativeByteBuffer data = cursor.byteBufferValue(0);
                            if (data != null) {
                                TLRPC.Message message = TLRPC.Message.TLdeserialize(data, data.readInt32(false), false);
    ...
    private static void saveReplyMessages(final HashMap<Integer, ArrayList<MessageObject>> replyMessageOwners, final ArrayList<TLRPC.Message> result) {
        MessagesStorage.getInstance(UserConfig.selectedAccount).getStorageQueue().postRunnable(new Runnable() {
            ...
            public void run() {
                try {
                    MessagesStorage.getInstance(UserConfig.selectedAccount).getDatabase().beginTransaction();
                    SQLitePreparedStatement state = MessagesStorage.getInstance(UserConfig.selectedAccount).getDatabase().executeFast("UPDATE messages SET replydata = ? WHERE mid = ?");
        ...
        AndroidUtilities.runOnUIThread(new Runnable() {
            ...
            public void run() {
                ...
                for (int a = 0; a < result.size(); a++) {
                    ...
                    if (arrayList != null) {
                        MessageObject messageObject = new MessageObject(UserConfig.selectedAccount , message, usersDict, chatsDict, false);
                        for (int b = 0; b < arrayList.size(); b++) {
                ...
                if (changed) {
                    NotificationCenter.getInstance(UserConfig.selectedAccount).postNotificationName(NotificationCenter.didLoadedReplyMessages, dialog_id);
                }
            }
        });
    }
        ...
        if (message[0] instanceof Spannable) {
            ...
            if (spansMentions != null && spansMentions.length > 0) {
                ...
                for (int b = 0; b < spansMentions.length; b++) {
                    ...
                    entity.user_id = MessagesController.getInstance(UserConfig.selectedAccount).getInputUser(Utilities.parseInt(spansMentions[b].getURL()));
